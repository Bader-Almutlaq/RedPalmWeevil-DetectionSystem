...
num_classes = 2
input_folder = "NRPWinsideTrap2000"     # Folder of images to process
output_folder = "output_crops"  # Where to save confident RPW crops
...
def main():
    ...
    for filename in os.listdir(input_folder):
        ...
        for idx, box in enumerate(boxes):
            ...
            crop_resized = cv2.resize(crop, (380, 380))  # Resize crop to 380x380
            ...
            if pred == 1 :
                ...
                crop_name = f"RPW_{os.path.splitext(filename)[0]}_{confidence:.2f}.jpg"
            ...
            if pred == 0 :
                crop_name = f"NRPW_{os.path.splitext(filename)[0]}_{confidence:.2f}.jpg"
                crop_path = os.path.join(output_folder, crop_name)
                cv2.imwrite(crop_path, crop_resized)
                print(f"[✔] {filename} → NRPW with {confidence:.2f}, saved: {crop_path}")
                
        if RPW_classification is None:
            print(f"[–] {filename} → No RPW > 0.00")
